...
- `Dim_Weather` currently only maps WMO codes observed in early data, not the full WMO list
- Currently a daily batch pipeline — future iteration will explore streaming ingestion via Kafka
- Currently runs against local PostgreSQL — future iteration will migrate to a managed cloud database